...
function Query8() {
    ...
    useEffect(() => {
      (async function () {
        let data = await fetch(`http://localhost:3001/Query8/${Date1}/${Date2}/${FName}/${SName}/${LName}`).then((res) => res.json());
        setInfo(data);
      })();
      ...
      infois = info.map((a) => {
        ...
        return (
          ...
          className="border pt-3 pb-3 border-dark  col-12 position-relative text-dark rounded mb-3 "
           key={SName[0]+FName[0]+LName[0]}>
                  <div className="fs-4 fw-normal text-center">{SName[0]} {FName[0]} {LName[0]}</div> 
         </div>)
      })
    }
  ...
  return (
<div className="modal fade" id="Query8" data-bs-backdrop="static" data-bs-keyboard="false" tabIndex="-1" aria-labelledby="Query8Label" aria-hidden="true">
  <div className="modal-dialog modal-lg">
    <div className="modal-content">
      <div className="modal-header">
        <h1 className="modal-title fs-5" id="Query8Label">Query8</h1>
        <button type="button" className="btn-close" data-bs-dismiss="modal" aria-label="Close"
        ></button>
      </div>
      <div className="modal-body justify-content-center">
      <div className="fw-semibold fw-5 text-center mb-3 ">Выдать список читателей, которые в течение обозначенного периода были обслужены указанным
библиотекарем.</div>
